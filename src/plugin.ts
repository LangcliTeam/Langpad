...
const DEFAULT_SETTINGS: AgentClientPluginSettings = {
	...
	nodePath: "",
	exportSettings: {
		defaultFolder: "Langpad",
		filenameTemplate: "langpad_{date}_{time}",
		autoExportOnNewChat: false,
		autoExportOnCloseChat: false,
		openFileAfterExport: true,
		includeImages: true,
		imageLocation: "obsidian",
		imageCustomFolder: "Langpad",
		frontmatterTag: "langpad",
	},
...
export default class AgentClientPlugin extends Plugin {
	...
	private broadcastPrompt(): void {
		...
		if (allViews.length === 0) {
			new Notice("[Langpad] No chat views open");
			return;
		}
		...
		if (
			...
			new Notice("[Langpad] No prompt to broadcast");
			return;
		}
		...
		if (targetViews.length === 0) {
			new Notice("[Langpad] No other chat views to broadcast to");
			return;
		}
	...
	private async broadcastSend(): Promise<void> {
		...
		if (sendableViews.length === 0) {
			new Notice("[Langpad] No views ready to send");
			return;
		}
	...
	private async broadcastCancel(): Promise<void> {
		...
		new Notice("[Langpad] Cancel broadcast to all views");
	}
	...
	async loadSettings() {
		...
		this.settings = {
			langcli: {
				id: D.langcli.id,
				displayName: str(rl.displayName, D.langcli.displayName),
				command: str(rl.command, "") || D.langcli.command,
				args:
					sanitizeArgs(rl.args).length > 0
						? sanitizeArgs(rl.args)
						: D.langcli.args,
				env: normalizeEnvVars(rl.env),
			},
			claude: {
	...
	async checkForUpdates(): Promise<boolean> {
		...
		if (isCurrentPrerelease) {
			...
			if (latestStable && semver.gt(latestStable, currentVersion)) {
				new Notice(`[Langpad] Update available: v${latestStable}`);
				return true;
			}
		}
	...
	ensureDefaultAgentId(): void {
		...
		if (availableIds.length === 0) {
			this.settings.defaultAgentId = DEFAULT_SETTINGS.langcli.id;
			return;
		}
	...
	private collectAvailableAgentIds(): string[] {
		...
		ids.add(this.settings.langcli.id);
