...
export function getDefaultAgentId(settings: AgentClientPluginSettings): string {
	return settings.defaultAgentId || settings.langcli.id;
}
...
export function getAvailableAgentsFromSettings(
	...
	return [
		{
			id: settings.langcli.id,
			displayName: settings.langcli.displayName || settings.langcli.id,
		},
		{
...
export function findAgentSettings(
	...
	if (agentId === settings.langcli.id) {
		return settings.langcli;
	}
...
export function buildAgentConfigWithApiKey(
	...
	// Langcli does not need an API key — it supports multi-model routing natively
	if (agentId === settings.langcli.id) {
		return baseConfig;
	}
