...
export class AgentClientSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		docContainer.createEl("a", {
			text: "documentation",
			href: "https://langcli.com/docs/ACP/obsidian",
			attr: { target: "_blank" },
		});
		...
		this.renderLangcliSettings(containerEl);
		...
		new Setting(containerEl)
			...
			.addText((text) =>
				text
					.setPlaceholder("langpad_{date}_{time}")
					.setValue(
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Tag to add to exported notes. Supports nested tags (e.g., projects/langpad). Leave empty to disable.",
			)
			.addText((text) =>
				text
					.setPlaceholder("langpad")
	...
	private renderLangcliSettings(sectionEl: HTMLElement) {
		const langcli = this.plugin.settings.langcli;

		new Setting(sectionEl)
			.setName(langcli.displayName || "Langcli")
			.setDesc(
				"Built-in AI agent with multi-model routing. Start with `langcli --acp` and chat with any AI model.",
			)
			.setHeading();

		const langcliPathSetting = new Setting(sectionEl)
			.setName("Path")
			.setDesc(
				'Command name or path to langcli. Use just "langcli" to let the login shell resolve it, or enter an absolute path.',
			)
			.addText((text) => {
				text.setPlaceholder("langcli")
					.setValue(langcli.command)
					.onChange(async (value) => {
						this.plugin.settings.langcli.command = value.trim();
						await this.plugin.saveSettings();
					});
			});
		this.addAutoDetectButton(
			...
			"langcli",
			async (path) => {
				this.plugin.settings.langcli.command = path;
				await this.plugin.saveSettings();
			},
		);
		this.addInstallHint(sectionEl, "@langcli/cli");

		new Setting(sectionEl)
			.setName("Arguments")
			.setDesc(
				"Enter one argument per line. By default, langcli starts in ACP mode with `--acp`.",
			)
			.addTextArea((text) => {
				text.setPlaceholder("--acp")
					.setValue(this.formatArgs(langcli.args))
					.onChange(async (value) => {
						this.plugin.settings.langcli.args =
							this.parseArgs(value);
						await this.plugin.saveSettings();
					});
				text.inputEl.rows = 3;
		...
		new Setting(sectionEl)
			.setName("Environment variables")
			.setDesc(
				"Enter KEY=VALUE pairs, one per line. Langcli does not require API keys by default.",
			)
			.addTextArea((text) => {
				text.setPlaceholder("")
					.setValue(this.formatEnv(langcli.env))
					.onChange(async (value) => {
						this.plugin.settings.langcli.env = this.parseEnv(value);
						await this.plugin.saveSettings();
					});
				text.inputEl.rows = 3;
			});
